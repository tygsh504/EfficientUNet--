...
# class BasicSegmentationDataset(Dataset):
#     def __init__(self, imgs_dir: str, masks_dir: str, scale: float = 1, mask_suffix: str = '', augmentation_ratio: int = 0, aug_policy: str = 'crop'):
#         self.imgs_dir = imgs_dir
#         self.masks_dir = masks_dir
#         self.scale = scale
#         self.mask_suffix = mask_suffix
...
#     @classmethod
#     def preprocess(cls, pil_img: Image, scale: float, is_mask: bool = False) -> np.array:
#         w, h = pil_img.size
#         newW, newH = int(scale * w), int(scale * h)
        
#         # Force dimensions to be multiples of 32 to prevent feature map size mismatch in U-Net
#         newW = int(round(newW / 32) * 32)
#         newH = int(round(newH / 32) * 32)
...
#         # Use nearest neighbor for masks to prevent interpolating class values
#         resample_method = Image.NEAREST if is_mask else Image.BILINEAR
#         pil_img = pil_img.resize((newW, newH), resample=resample_method)
...
#         if is_mask:
#             img_trans = np.where(img_trans > 0.5, 1.0, 0.0).astype(np.float32)

#         return img_trans

#     def augment(self, image, mask, policy='crop', augmentation_ratio=0):
#         tf_imgs = []
#         tf_masks = []
        
#         for i in range(augmentation_ratio): 
#             if policy == 'crop':
#                 aug_policy = CropPolicy() 
#             else:
#                 aug_policy = RetinaPolicy()
       
#             tf_image, tf_mask = aug_policy(image, mask) 
              
#             tf_image = self.preprocess(tf_image, self.scale, is_mask=False)
#             tf_mask = self.preprocess(tf_mask, self.scale, is_mask=True)
            
#             tf_imgs.append(torch.from_numpy(tf_image).type(torch.FloatTensor))
#             tf_masks.append(torch.from_numpy(tf_mask).type(torch.FloatTensor))

#         orig_image = self.preprocess(image, self.scale, is_mask=False)
#         orig_mask = self.preprocess(mask, self.scale, is_mask=True)
        
#         tf_imgs.insert(0, torch.from_numpy(orig_image).type(torch.FloatTensor))
#         tf_masks.insert(0, torch.from_numpy(orig_mask).type(torch.FloatTensor))
...
#         images, masks = self.augment(img, mask, policy=self.policy, augmentation_ratio=self.augmentation_ratio)
...
#         assert image.size == mask.size, f'Image and mask {idx} should be the same size, but are {image.size} and {mask.size}'        

#         images, masks = self.augment(image, mask, policy = self.policy, augmentation_ratio = self.augmentation_ratio)
#         return {'image': images, 'mask': masks}

# class CoronaryArterySegmentationDataset(BasicSegmentationDataset):
#     gray2class_mapping = {0: 0, 100: 1, 255: 2}
#     gray2rgb_mapping = {0: (0, 0, 0), 100: (255, 0, 0), 255: (255, 255, 255)}
#     rgb2class_mapping = {(0, 0, 0): 0, (255, 0, 0): 1, (255, 255, 255): 2}
#     n_classes = 3
