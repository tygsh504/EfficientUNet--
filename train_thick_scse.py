...
#             # --- MODIFIED FOR "THICK" SCSE ---
#             # Force the intermediate channels (mip) to equal the input channels,
#             # bypassing any standard reduction bottleneck.
#             mip = in_channels 
#             # ---------------------------------
...
#         Mixed Precision: Enabled (Modern)
#     ''')
...
#                 # Runs the forward pass with modern autocasting.
...
def replace_attention_with_scse(model):
    ...
    for child_name, child in model.named_children():
        ...
        if hasattr(child, 'conv_h') and hasattr(child, 'conv_w') and hasattr(child, 'conv1'):
            ...
            # --- MODIFIED FOR "THICK" SCSE USING NATIVE SMP MODULE ---
            # The smp.base.modules.SCSEModule uses a 'reduction' parameter. 
            # To force it to be "thick" (no bottleneck, intermediate channels = in_channels), 
            # we set reduction=1.
            scse = smp.base.modules.SCSEModule(in_channels, reduction=1)
            # ---------------------------------------------------------
