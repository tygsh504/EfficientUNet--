...
def train_net(net,
    ...
    # ==========================================
    # PROGRESSIVE UNFREEZING: STEP 1
    # Freeze encoder parameters for the first 10 epochs
    # ==========================================
    logging.info("Freezing encoder parameters for the first 10 epochs...")
    ...
    for epoch in range(epochs):
        ...
        # ==========================================
        # PROGRESSIVE UNFREEZING: STEP 2
        # Unfreeze at Epoch 10 and drop LR by 10x
        # ==========================================
        if epoch == 10:
            logging.info("Epoch 10: Unfreezing encoder and reducing LR by 10x for fine-tuning.")
            encoder = net.module.encoder if hasattr(net, 'module') else net.encoder
            for param in encoder.parameters():
                param.requires_grad = True
            
            # Reduce the learning rate by 10x
            for param_group in optimizer.param_groups:
                param_group['lr'] = param_group['lr'] / 10.0
                
            # Reset the scheduler so it starts its "patience" count fresh with the new LR
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5)
...
if __name__ == '__main__':
    ...
    # (Encoder freezing/unfreezing is handled inside train_net)
    net.train()
