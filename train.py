...
def train_net(net,
    ...
    logging.info(f'''Starting training:
        ...
        Augmentation ratio: {augmentation_ratio}
        Classes:         {n_classes}
        Encoder:         Progressive Unfreezing (Frozen initially)
        Mixed Precision: Enabled
    ''')

    optimizer = optim.Adam(net.parameters(), lr=lr)
    
    # --- MODIFICATION 4: COSINE ANNEALING LR ---
    # Smoothly decays the learning rate over the total number of epochs
    ...
    for epoch in range(epochs):
        ...
        # --- MODIFICATION 3: PROGRESSIVE UNFREEZING (Unfreeze step) ---
        # Unfreeze the encoder at Epoch 10 to fine-tune the whole model
        if epoch == 10:
            logging.info("Epoch 10 reached: Unfreezing encoder for fine-tuning...")
            # Handle potential DataParallel wrapping safely
            model_to_unfreeze = net.module if hasattr(net, 'module') else net
            
            for param in model_to_unfreeze.encoder.parameters():
                param.requires_grad = True
                
            # Gently lower the learning rate so we don't damage the pre-trained weights
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with autocast():
                    ...
                    if n_classes == 1:
                        # --- MODIFICATION 1: TACKLE CLASS IMBALANCE ---
                        # Use a pos_weight to heavily penalize missing small disease lesions (False Negatives)
        ...
        # --- MODIFICATION 4: COSINE ANNEALING LR ---
        # Step the scheduler (CosineAnnealingLR does not require val_score)
        scheduler.step()
        # -------------------------------------------
...
if __name__ == '__main__':
    ...
    # --- MODIFICATION 3: PROGRESSIVE UNFREEZING (Freeze step) ---
    # We turn off gradient calculation for the encoder so it stays frozen initially
