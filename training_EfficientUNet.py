...
def train_net(net,
    ...
    logging.info(f'''Starting training (Standard Precision):
        Epochs:          {epochs}
        Batch size:      {batch_size}
        Learning rate:   {lr}
        Mixed Precision: Disabled (Standard FP32)
        Architecture:    Standard EfficientUNet
    ''')
...
''')
    ...
    # --- SAVE EXCEL & GENERATE PLOTS ---
    # 1. Save training history to Excel
    pd.DataFrame(history).to_excel(os.path.join(dir_checkpoint, 'training_metrics.xlsx'), index=False)
    logging.info(f"Training metrics saved to {os.path.join(dir_checkpoint, 'training_metrics.xlsx')}")
    
    # 2. Plotting logic for Loss, Learning Rate, and Dice Coeff
    ...
    plt.subplot(1, 3, 3)
    plt.plot(history['val_dice'], label='Val Dice', color='red')
    plt.plot(history['train_dice'], label='Train Dice', color='pink', linestyle='--')
    plt.title('Dice Coefficient over Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Dice Coeff')
    plt.legend()
    
    # Save the plotted graph as a .jpg
    plt.savefig(os.path.join(dir_checkpoint, 'training_plot.jpg'))
    plt.close()
    logging.info(f"Training plots saved to {os.path.join(dir_checkpoint, 'training_plot.jpg')}")
...
if __name__ == '__main__':
    ...
    # Instantiate standard EfficientUNet
    net = smp.Unet(encoder_name=args.encoder, encoder_weights="imagenet", in_channels=3, classes=n_classes)
