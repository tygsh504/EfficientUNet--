...
def tversky_loss_fn(inputs, targets, alpha=0.3, beta=0.7, smooth=1e-6):
...
def train_net(net,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with autocast('cuda'):
                    ...
                    if n_classes == 1:
                        # --- MODIFIED: TVERSKY LOSS FOR SEVERE IMBALANCE ---
                        # Alpha (FP penalty) = 0.3, Beta (FN penalty) = 0.7 
                        # High beta forces it to find the tiny disease dots
                        loss = tversky_loss_fn(masks_pred, true_masks, alpha=0.3, beta=0.7)
                        loss += dice_loss(masks_pred, true_masks).mean()
                        # ----------------------------------------------------

                        # Calculate metrics for the batch safely using sigmoid
                        ...
                        loss += smp.losses.DiceLoss(mode='multiclass')(masks_pred, true_masks.squeeze(1))
        ...
        avg_train_loss = epoch_loss / max(1, (n_train // (batch_size / (1 + augmentation_ratio))))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'DRIVE':
        ...
        # --- MODIFIED: FORCED CROP AUGMENTATION FOR TINY DOTS ---
        training_set = BasicSegmentationDataset(
            imgs_dir=args.train_img_dir, 
            masks_dir=args.train_mask_dir, 
            scale=args.scale,
            mask_suffix='',
            augmentation_ratio=1,     # Enforce at least 1 to trigger proper mixing
            aug_policy='crop'         # Enforce the crop policy to avoid resizing artifacts
        )
